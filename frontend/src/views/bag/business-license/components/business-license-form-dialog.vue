  ...
  >
    <el-form ref="form" label-position="right" :model="form" :rules="rules">
      <el-row>
        ...
        <el-col :span="24">
          <el-form-item label="营业期限" :label-width="labelWidth" prop="content.businessTerm">
            <el-date-picker
              v-model="form.content.businessTerm"
        ...
        <el-col :span="24">
          <el-form-item label="统一社会信用代码" :label-width="labelWidth" prop="content.unifiedSocialCreditCode">
            <el-input v-model="form.content.unifiedSocialCreditCode" autocomplete="off" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="营业执照照片" label-width="auto" prop="content.businessLicensePhoto1">
            <el-upload
              class="avatar-uploader"
              action="#"
              accept="image/png, image/jpeg"
              :show-file-list="false"
              :on-success="(response, file, fileList) => handleUploadSuccess(response, file, fileList, 'businessLicensePhoto1')"
              :on-error="handleUploadError"
              :on-progress="handleUploadProgress"
              :http-request="(content) => upload(content, 'businessLicensePhoto1' )"
            >
              <img v-if="form.content.businessLicensePhoto1" :src="VUE_APP_BASE_API + form.content.businessLicensePhoto1" class="avatar">
              <i v-else class="el-icon-plus avatar-uploader-icon" />
              <div v-show="uploadProgressPercentbusinessLicensePhoto1 !== 0" class="progress-wrapper">
                <el-progress type="circle" :percentage="uploadProgressPercentbusinessLicensePhoto1" />
              </div>
            </el-upload>
          </el-form-item>
        </el-col>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        'content.businessTerm': [{ required: true, message: '必填', trigger: 'blur' }],
        'content.unifiedSocialCreditCode': [{ required: true, message: '必填', trigger: 'blur' }],
        'content.businessLicensePhoto1': [{ required: true, message: '必填', trigger: 'change' }]
      },
      loading: false,
      saving: false,
      // 图片上传
      uploadProgressPercentbusinessLicensePhoto1: 0,
      // 业务属性
      loadingSelect: false,
      userList: []
    }
  ...
  computed: {
    ...
    title() {
      return this.form.id === '-1' ? '添加营业执照' : '编辑营业执照'
    }
  },
  created() {
    // 拷贝form默认值
  ...
  methods: {
    ...
    handleUploadSuccess(response, file, fileList, customParam) {
      // this.form.businessLicensePhoto1 = response.data.data.url
