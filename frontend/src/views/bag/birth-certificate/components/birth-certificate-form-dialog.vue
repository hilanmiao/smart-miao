  ...
  >
    <el-form ref="form" label-position="right" :model="form" :rules="rules">
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item label="出生时间" :label-width="labelWidth" prop="content.birthTime">
            <el-date-picker
              v-model="form.content.birthTime"
              type="datetime"
        ...
        <el-col :span="12">
          <el-form-item label="出生证照片" label-width="auto" prop="content.birthCertificatePhoto1">
            <el-upload
              class="avatar-uploader"
              action="#"
              accept="image/png, image/jpeg"
              :show-file-list="false"
              :on-success="(response, file, fileList) => handleUploadSuccess(response, file, fileList, 'birthCertificatePhoto1')"
              :on-error="handleUploadError"
              :on-progress="handleUploadProgress"
              :http-request="(content) => upload(content, 'birthCertificatePhoto1' )"
            >
              <img v-if="form.content.birthCertificatePhoto1" :src="VUE_APP_BASE_API + form.content.birthCertificatePhoto1" class="avatar">
              <i v-else class="el-icon-plus avatar-uploader-icon" />
              <div v-show="uploadProgressPercentBirthCertificatePhoto1 !== 0" class="progress-wrapper">
                <el-progress type="circle" :percentage="uploadProgressPercentBirthCertificatePhoto1" />
              </div>
            </el-upload>
          </el-form-item>
        </el-col>
...
export default {
    ...
    return {
      ...
      form: {
        ...
        content: {
          name: '',
          birthTime: '',
          birthCertificatePhoto1: ''
        }
      },
      rules: {
        'content.name': [{ required: true, message: '必填', trigger: 'blur' }],
        'content.birthTime': [{ required: true, message: '必填', trigger: 'blur' }],
        'content.birthCertificatePhoto1': [{ required: true, message: '必填', trigger: 'change' }]
      },
      loading: false,
      saving: false,
      // 图片上传
      uploadProgressPercentBirthCertificatePhoto1: 0,
      // 业务属性
      loadingSelect: false,
      userList: []
    }
  ...
  computed: {
    ...
    title() {
      return this.form.id === '-1' ? '添加出生证' : '编辑出生证'
    }
  },
  created() {
    // 拷贝form默认值
  ...
  methods: {
    ...
    handleUploadSuccess(response, file, fileList, customParam) {
      // this.form.birthCertificatePhoto1 = response.data.data.url
